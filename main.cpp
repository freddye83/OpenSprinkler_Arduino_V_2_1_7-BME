...
EthernetClient *m_client = 0;
  ...
  unsigned long TimerTmp=0;           // compteur mesure température
  const unsigned long tmpsMesureTmp=6000;   // Temps boucle mesure température 
#endif
...
void do_setup() {
  ...
  DEBUG_BEGIN(115200);
  //OPENSPRINKLER_ARDUINO_DEBUG
  DEBUG_PRINTLN(F("OpenSprinkler is starting"));
    ...
    bme_status = bme.begin();  
      if (!bme_status) {
        ...
        DEBUG_PRINTLN ("Capteur BME trouvé !");
      }
      // bme.seaLevelForAltitude(0,1013.25);
#endif
...
#ifdef OPENSPRINKLER_BME280 // Utilisation d'un capteur BME280 I2C
  float bme_f;
    if ((millis() - TimerTmp > tmpsMesureTmp)) {                //rafraichissement temmpérature 60s

          bme_f=bme.readTemperature()-1;
          dtostrf(bme_f,6,2,os.temp);
          bme_f=bme.readPressure()/ 100.0F;
          dtostrf(bme_f,0,2,os.pression);        
          bme_f=bme.readHumidity();
          dtostrf(bme_f,6,2,os.humidite);
...
/*          
  ...
  if (curr_time != last_time) {
    ...
    #ifdef OPENSPRINKLER_ARDUINO_HEARTBEAT
        digitalWrite(PIN_HEARTBEAT, curr_time % 2);
    #endif // OPENSPRINKLER_ARDUINO_HEARTBEAT
  }
...
void perform_ntp_sync() {
  ...
  if (os.status.req_ntpsync) {
    ...
    DEBUG_PRINT(F("NTP Syncing... ")); // OPENSPRINKLER_ARDUINO_DEBUG
    if (!ui_state) {
    ...
    if (t>0) {
      ...
      DEBUG_PRINTLN(F("(OK)")); // OPENSPRINKLER_ARDUINO_DEBUG
    }
    else
        DEBUG_PRINTLN(F("(failed)")); // OPENSPRINKLER_ARDUINO_DEBUG
  }
#else
  // nothing to do here
