...
#ifdef OPENSPRINKLER_BME280 // Utilisation d'un capteur BME280 I2C
   char OpenSprinkler::temp[10];
   char OpenSprinkler::pression[12];
   char OpenSprinkler::humidite[10];
#endif    
...
bool OpenSprinkler::read_hardware_mac() {
  ...
  return true;
#else
    #if defined(OPENSPRINKLER_ARDUINO_W5100) || defined (OPENSPRINKLER_ARDUINO_ENC28J60)
...
byte OpenSprinkler::start_network() {
...
#ifdef SERIAL_DEBUG //OPENSPRINKLER_ARDUINO_DEBUG
    ether.printMac(F("MAC is "), tmp_buffer);
#endif              //OPENSPRINKLER_ARDUINO_DEBUG

    if (!ether.begin(ETHER_BUFFER_SIZE, (uint8_t*)tmp_buffer, PIN_ETHER_CS))
    {
        DEBUG_PRINTLN(F("Error starting ethernet"));    //OPENSPRINKLER_ARDUINO_DEBUG
        return 0;
    }
    ...
    DEBUG_PRINT(F("Port:\t"));    //OPENSPRINKLER_ARDUINO_DEBUG
    DEBUG_PRINTLN(ether.hisport); //OPENSPRINKLER_ARDUINO_DEBUG
    ...
    if (options[OPTION_USE_DHCP]) {
        ...
        options_save();
        DEBUG_PRINTLN(F("DCHP IP address:")); //OPENSPRINKLER_ARDUINO_DEBUG
    } else {
        // set up static IP
        ...
        DEBUG_PRINTLN(F("Static IP address:")); //OPENSPRINKLER_ARDUINO_DEBUG
    }
...
void OpenSprinkler::apply_all_station_bits() {
  ...
  byte bid, s, sbits;
    ...
    // do the station processing seperately - its just easier
    OpenSprinklerStation.apply(status.enabled, station_bits, MAX_EXT_BOARDS);
    byte bid, s, sbits;
...
{
#ifndef ESP8266     // OPENSPRINKLER_ARDUINO_FIXED_ERROR (was originally #ifdef)
  lcd.setCursor(0, 0);
