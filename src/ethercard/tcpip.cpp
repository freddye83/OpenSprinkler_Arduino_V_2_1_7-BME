...
static uint8_t tcp_result_cb(uint8_t fd, uint8_t status, uint16_t datapos, uint16_t datalen) {
    if (status == 0) {
        ...
        result_ptr = (char*) ether28j60.buffer + datapos;
...
uint16_t EtherCard::packetLoop (uint16_t plen) {
    ...
    if(using_dhcp){
        ether28j60.DhcpStateMachine(plen);
    }
    ...
    if (ether28j60.udpServerListening() && gPB[IP_PROTO_P]==IP_PROTO_UDP_V)
    {   //Call UDP server handler (callback) if one is defined for this packet
        if(ether28j60.udpServerHasProcessedPacket(plen))
