...
void Stash::load (uint8_t idx, uint8_t blk) {
    if (blk != bufs[idx].bnum) {
        if (idx == 0) {
            ether28j60.copyout(bufs[idx].bnum, bufs[idx].bytes);
            if (blk == bufs[1].bnum)
        ...
        bufs[idx].bnum = blk;
        ether28j60.copyin(bufs[idx].bnum, bufs[idx].bytes);
    }
}
...
void Stash::release () {
    while (first > 0) {
        freeBlock(first);
        first = ether28j60.peekin(first, 63);
    }
}
