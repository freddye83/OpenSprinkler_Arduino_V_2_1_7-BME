...
static void dnsRequest (const char *hostname, bool fromRam) {
    ...
    if (ether28j60.dnsip[0] == 0)
        memset(ether28j60.dnsip, 8, 4); // use 8.8.8.8 Google DNS as default
    ether28j60.udpPrepare((DNSCLIENT_SRC_PORT_H << 8) | dnstid_l, ether28j60.dnsip, 53);
...
static bool checkForDnsAnswer (uint16_t plen) {
    ...
    for (;;) {
        ...
        if (p[1] == 1 && p[9] == 4) { // type "A" and IPv4
            ether28j60.copyIp(ether28j60.hisip, p + 10);
            break;
        }
        p += p[9] + 10;
    }
